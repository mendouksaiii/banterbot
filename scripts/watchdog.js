...
• Monitors Banterbot process health every 30s
• Auto-restarts on crash, memory stall, or zombie connection
────────────────────────────────────────────────────────
`);

function startProcess() {
  console.log(`[${new Date().toLocaleTimeString()}] 🚀 Launching Banterbot child process...`);
  ...
  childProcess.on("exit", (code, signal) => {
    console.warn(`[${new Date().toLocaleTimeString()}] ⚠️ Banterbot process exited (code: ${code}, signal: ${signal}).`);
    childProcess = null;
...
function handleHealthFailure(reason) {
  consecutiveFailures++;
  console.warn(`[${new Date().toLocaleTimeString()}] ⚠️ Warning: Banterbot unresponsive (${reason}). Strike ${consecutiveFailures}/${MAX_FAILURES}`);

  if (consecutiveFailures >= MAX_FAILURES) {
    console.error(`[${new Date().toLocaleTimeString()}] 🚨 Banterbot reached ${MAX_FAILURES} failed health checks! Force-killing zombie process...`);
